...
def load():
    ...
    if not ref_path.exists():
        # Generate on first request if not already done
...
async def perf_timeline():
    ...
    for day in range(7, 30, 3):
        ...
        snaps.append({
            "day": day,
            "date": (datetime.utcnow() - timedelta(days=30 - day)).strftime("%m/%d"),
            "auc_roc": s.auc_roc, "precision": s.precision,
            "recall": s.recall, "f1_score": s.f1_score,
...
async def alerts():
    ...
    return [
        Alert(
            alert_id=str(uuid.uuid4()), alert_type="drift",
...
async def retrain(body: RetrainTrigger):
    return RetrainStatus(job_id=str(uuid.uuid4()), status="queued",
                         triggered_at=datetime.utcnow(), reason=body.reason)
...
async def comparison():
    ...
    return ModelComparison(
        model_a="v1.2.0 (production)", model_b="v1.3.0-challenger (shadow)",
