...
FEATURES = ["amount","amount_zscore","txn_count_1h","txn_count_24h","hour_of_day","is_weekend","is_online","is_foreign","merchant_risk","card_age_days","unique_merch_24h"]

def load():
    return pd.read_parquet(DATA_DIR/"reference.parquet"), pd.read_parquet(DATA_DIR/"production.parquet")
...
async def drift_report(days: int = Query(default=7, ge=1, le=30)):
    ...
    return DriftDetector(ref, FEATURES).compute_drift(recent, datetime.utcnow()-timedelta(days=days), datetime.utcnow())
...
async def drift_timeline():
    ...
    for day in range(0, 30, 3):
        ...
        tl.append({"day":day,"date":(datetime.utcnow()-timedelta(days=30-day)).strftime("%m/%d"),
                   "overall_status":r.overall_drift_status.value,
                   "features":{f.feature_name:{"psi":f.psi,"status":f.drift_status.value} for f in r.features}})
    return {"timeline": tl}
...
async def perf_timeline():
    ...
    for day in range(7, 30, 3):
        ...
        snaps.append({"day":day,"date":(datetime.utcnow()-timedelta(days=30-day)).strftime("%m/%d"),
                      "auc_roc":s.auc_roc,"precision":s.precision,"recall":s.recall,
                      "f1_score":s.f1_score,"avg_score":s.avg_prediction_score,
                      "high_risk_rate":s.high_risk_rate,"sample_count":s.sample_count})
...
async def alerts():
    ...
    return [Alert(alert_id=str(uuid.uuid4()),alert_type="drift",
                  ...
                  metric_name="psi",metric_value=f.psi,
...
async def retrain(body: RetrainTrigger):
    return RetrainStatus(job_id=str(uuid.uuid4()),status="queued",triggered_at=datetime.utcnow(),reason=body.reason)
...
async def comparison():
    ...
    return ModelComparison(model_a="v1.2.0 (production)",model_b="v1.3.0-challenger (shadow)",
        comparison_period_days=7,sample_count_a=len(sa),sample_count_b=len(sb),
        auc_a=0.987,auc_b=0.991,f1_a=0.902,f1_b=0.914,
        ...
        winner="v1.3.0-challenger",confidence=0.94,
    ...
    try:
        while True:
            ...
            await websocket.send_json(LivePrediction(
                ...
                score=round(score,4),risk_level=risk,
                latency_ms=round(float(rng.gamma(2,4)),1),
                timestamp=datetime.utcnow(),model_version="v1.2.0"
